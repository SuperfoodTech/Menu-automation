...
from shopee.core.edit import _boot_client
...
class ShopeeAdapter(PlatformAdapter):
    ...
    def sync_changes(self, db, outlet, pending_categories, pending_dishes) -> list[str]:
        ...
        from shopee.core.create import create_category, create_dish
        from shopee.core.edit import update_category, update_dish
        ...
        for cat in pending_categories:
            if cat.sync_status == "pending_create":
                ...
                if new_cat_data and "id" in new_cat_data:
                    cat._new_id = str(new_cat_data["id"])
                    cat.sync_status = "synced"
                    db.commit()
                    print(f"  [Sync] Category '{cat.name}' created on Shopee.")
                else:
